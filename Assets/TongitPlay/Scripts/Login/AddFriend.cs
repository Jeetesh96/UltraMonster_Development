...
public class AddFriend : MonoBehaviourPunCallbacks
{
    ...
    public IEnumerator AddFirstFriendRelationToDB()
    {
        ...
        WWW w = new WWW("http://65.1.1.23/gameserver/gameserver/action_add_friend.php", form);
        yield return w;
    ...
    public IEnumerator DeleteFrFromDB()
    {
        ...
        WWW w = new WWW("http://65.1.1.23/gameserver/gameserver/delete_friend_req", form);
        yield return w;
        Debug.Log(w.text.ToString());
        if (string.IsNullOrEmpty(w.error))
        {
